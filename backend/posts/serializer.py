...
from shared.models import Post, Tag , Project, Event  # Adjust import if needed
...
class PostSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if post_type == "project":
            post_instance = Project.objects.create(**validated_data, github_link=github_link)
        elif post_type == "event":
            if github_link:  # Ensure event posts don't have a github_link
                raise serializers.ValidationError({"github_link": "This field is only allowed for projects."})
            post_instance = Event.objects.create(**validated_data)
        else:
